...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator_content = $('<div class="flowchart-operator-content"></div>');
            $operator_content.html(infos.content);
            $operator_content.appendTo($operator);
        ...
        setOperatorContent: function (operatorId, content) {
            this.data.operators[operatorId].internal.els.content.html(content);
            if (typeof this.data.operators[operatorId].properties == 'undefined') {
                this.data.operators[operatorId].properties = {};
            }
            this.data.operators[operatorId].properties.content = content;
            this._refreshInternalProperties(this.data.operators[operatorId]);
            this.options.onAfterChange('operator_content_change');
        },

        getOperatorContent: function (operatorId) {
            var content = this.data.operators[operatorId].internal.properties.content;
